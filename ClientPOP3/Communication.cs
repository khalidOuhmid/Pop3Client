...
namespace ClientPOP3
{
    public static class Communication
    {
        ...
        public static void RetrInfoAll()
        {
            string ligne, tampon;
            tampon = "STAT";
            EcrireLigne(tampon);
            ligne = LireLigne();
                string[] lesValeurs = ligne.Split(' ');
                int nombreMessages = Int32.Parse(lesValeurs[1]);
            for (int i = 1; i < nombreMessages+1; i++)
            {
                TopInfo(i);
            }
        ...
        public static void TopInfo(int num)
        {
            string ligne, tampon;
            tampon = "top " + num +" " +  0;
            EcrireLigne(tampon);
            ...
            else
            {
                ...
                while (!ligne.Equals("."))
                {
                        ...
                        {
                            String mes = ligne.Substring(4);
                            string tmp = mes.Insert(0, "Ecrit le");
                            clientPOP3.WriteAffichage(tmp);
                        }
                        else if (ligne.StartsWith("From"))
                    {
                        String mes = ligne.Substring(4);
                        string tmp = mes.Insert(0, "Expéditeur");
                        clientPOP3.WriteAffichage(tmp);
                    }else if (ligne.StartsWith("Subject"))
                    {
                        String mes = ligne.Substring(7);
                        string tmp = mes.Insert(0, "Sujet");
                        clientPOP3.WriteAffichage(tmp);
                    }
